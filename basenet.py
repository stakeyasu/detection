...
def homework(train_X, train_y, test_X):
  ...
  fp1 = open("basenet.txt", "w")
  n_ch = 64
    
  layers = [                            # (縦の次元数)x(横の次元数)x(チャネル数)
    Conv_BN_DROPOUT((3, 3, 3, n_ch), tf.nn.relu, [1,1,1,1]),  # 32 -> 30
    Conv_BN_DROPOUT((3, 3, n_ch, n_ch), tf.nn.relu, [1,1,1,1]),  # 30 -> 28
    Conv_BN_DROPOUT((2, 2, n_ch, 2*n_ch), tf.nn.relu, [1,2,2,1]),  # 28 -> 14
    Conv_BN_DROPOUT((3, 3, 2*n_ch, 2*n_ch), tf.nn.relu, [1,1,1,1]),  # 14 -> 12
    Conv_BN_DROPOUT((3, 3, 2*n_ch, 2*n_ch), tf.nn.relu, [1,1,1,1]),  # 12 ->  10
    Conv_BN_DROPOUT((2, 2, 2*n_ch, 4*n_ch), tf.nn.relu, [1,2,2,1]),  #  10 ->  5
    Conv_BN_DROPOUT((3, 3, 4*n_ch, 4*n_ch), tf.nn.relu, [1,1,1,1]),  #  5 ->  3
    Pooling((1, 3, 3, 1)),            #  3 ->  1
    Flatten(),
    Dense(4*n_ch, 10, tf.nn.softmax)
  ]
