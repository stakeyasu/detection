...
def homework(train_X, train_y, test_X):
  ...
  class DW_RESNET_MODULE:
    def __init__(self, filter_shape, function, strides, padding='VALID'):
        ...
        self.mid_ch = int(filter_shape[2]*4)
    ...
    def f_prop(self, x, keep_prob):
        ...
        h=Conv_BN((1, 1, self.in_ch, self.mid_ch), self.function, self.strides, self.padding).f_prop(x,keep_prob)
        h=dwConv_BN((self.kernel, self.kernel, self.mid_ch, 1), self.function, [1,1,1,1], self.padding).f_prop(h,keep_prob)
        ...
        if (self.in_ch==self.out_ch)and(self.strides==[1,1,1,1]):
	        return tf.nn.relu(x+h)
                ...
                h2=Conv_BN((1, 1, self.in_ch, self.out_ch), self.function, self.strides, self.padding).f_prop(x,keep_prob)
