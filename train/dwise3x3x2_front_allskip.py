...
def homework(train_X, train_y, test_X):
  ...
  class DW_RESNET_MODULE:
    ...
    def f_prop(self, x, keep_prob):
        ...
        h=Conv_BN((1, 1, self.in_ch, self.out_ch),self.function, [1,1,1,1], self.padding).f_prop(x,keep_prob)
        h=dwConv_BN((self.kernel, self.kernel, self.out_ch, 1),self.function, self.strides, self.padding).f_prop(h,keep_prob)
        h=dwConv_BN((self.kernel, self.kernel, self.out_ch, 1),"NON", [1,1,1,1], self.padding).f_prop(h,keep_prob)
