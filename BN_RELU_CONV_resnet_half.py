...
def homework(train_X, train_y, test_X):
  ...
  class BN_RELU_CONV:
    ...
    def f_prop(self, x, keep_prob):
        ...
        bn = tf.nn.batch_normalization(x, mean, var, None, None, 2e-5)
        func = self.function(bn)
        u = tf.nn.conv2d(func, self.W, self.strides, self.padding) + self.b
        return u
  ...
  class RESNET_MODULE:
    def __init__(self, filter_shape, function, strides, padding='VALID'):
        ...
        self.mid_ch = int(filter_shape[2]*0.5)
    ...
    def f_prop(self, x, keep_prob):
        ...
        h=BN_RELU_CONV((1, 1, self.in_ch, self.mid_ch), self.function, self.strides, self.padding).f_prop(x,keep_prob)
        h=BN_RELU_CONV((3, 3, self.mid_ch, self.mid_ch), self.function, [1,1,1,1], self.padding).f_prop(h,keep_prob)
        h=BN_RELU_CONV((1, 1, self.mid_ch, self.out_ch),self.function, [1,1,1,1], self.padding).f_prop(h,keep_prob)
